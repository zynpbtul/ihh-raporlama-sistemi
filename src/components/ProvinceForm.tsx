...
// Sadece senin belirlediğin 5 şehir kalsın
const PROVINCE_UNIVERSITIES: Record<string, string[]> = {
  'Ankara': ['Ankara Üniversitesi', 'ODTÜ', 'Hacettepe Üniversitesi', 'Gazi Üniversitesi', 'Bilkent Üniversitesi'],
  'İstanbul': ['İstanbul Üniversitesi', 'Boğaziçi Üniversitesi', 'İTÜ', 'Marmara Üniversitesi', 'Yıldız Teknik Üniversitesi'],
  ...
  'İzmir': ['Ege Üniversitesi', 'Dokuz Eylül Üniversitesi', 'İYTE', 'Yaşar Üniversitesi', 'İzmir Ekonomi Üniversitesi'],
  'Antalya': ['Akdeniz Üniversitesi', 'Alanya Alaaddin Keykubat Üniversitesi', 'Antalya Bilim Üniversitesi', 'Alanya Üniversitesi', 'Antalya Belek Üniversitesi']
};
...
export default function ProvinceForm({ province, onSaveRecords, onLogout }: ProvinceFormProps) {
  const [selectedWeek, setSelectedWeek] = useState('1. Hafta');
  const [selectedPeriod, setSelectedPeriod] = useState('Dönem Bir (Eylül, Ekim, Kasım, Aralık, Ocak)');
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setSuccessMessage(`${selectedWeek} verileri başarıyla kaydedildi! (Supabase entegrasyonu için şema hazırlandı)`);
    setTimeout(() => {
            ...
            {meetingLocationCount === 0 ? (
              ...
              <div className="space-y-3">
                ...
                {meetingEntries.map((entry, index) => {
                  const uniOptions = PROVINCE_UNIVERSITIES[province] || [];
