...
// Merkez paneli sorgusu
const fetchAllReports = async () => {
  // RLS kuralı merkez için 'true' (tümünü gör) olmalı
  const { data, error } = await supabase
    .from('raporlar')
    .select('*, rapor_detaylari(*)'); // Hem raporu hem detaylarını getirir
    
  if (data) setRecords(data);
};

export default function HqReport({ records, onLogout }: HqReportProps) {
  ...
  const provincesInDb = Array.from(new Set([...PROVINCES, ...currentRecords.map((r) => r.province)]));
  ...
  const provinceSummaries = provincesInDb.map((prov) => {
    ...
    const meetings = provRecords.filter((r) => r.activityType === 'meeting');
    const lessons = provRecords.filter((r) => r.activityType === 'lesson');
